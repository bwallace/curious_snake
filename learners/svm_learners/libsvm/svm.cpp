	...
	double *alpha, Solver::SolutionInfo* si, double Cp, double Cn)
{
	...
	// here we set the |w|^2 property (for extracting the distance to the hyperplane)
	// See: http://www.csie.ntu.edu.tw/~cjlin/libsvm/faq.html#f4151
	//
...
void svm_get_model_SVs(struct svm_model *model, struct svm_node** out_SVs)
{
    memcpy(out_SVs, model->SV[0], sizeof(struct svm_node) * model->nSV[0]);
}

double distance_to_h(struct svm_model *model, struct svm_node* x)
{
    ...
    return dec_values[0]/model->w_2;
}



double distance_of_closest_SV(struct svm_model *model)
{
    double closest = 10000;
    // model->l is total SVs
    for (int i=0; i < model->l; i++)
    {
        double curDist = distance_to_h(model, model->SV[i]);
        if (curDist < 0)
        {
            curDist = -1 * curDist;
        }
...
int get_num_SVs(struct svm_model *model)
{
    return model->nSV[0] + model->nSV[1];
}
...
double svm_kernel_function(const svm_model *model, const svm_node *x, const svm_node *y)
{
    return Kernel::k_function(x, y, model->param);
}
